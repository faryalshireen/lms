...
<?php
if (isset($_POST['a_announcement_submit']) && ($_SERVER["REQUEST_METHOD"] == "POST")) {
    ...
    $inserted = "INSERT INTO `announcements` (`role_id`,`date`,`title`,`comments`,`user_id`,`course_id`, `semester`, `assign_id`) VALUES ('$c_role_id','$current_date','$u_title','$u_comments','$teacherId', '$courseId', '$course_semester', '$assignId')";
    ...
    if ($query) {
        ...
        <script>
            setTimeout(() => {
                window.location = `admin_announcement.php`;
            }, 1000);
        </script>
        <?php
    } else {
...
<body>
    ...
    <div class="container">
        <div class="row">
            <form action="admin_create_announcement.php" method="POST" enctype="multipart/form-data">
                <div class="mt-3 col-md-7 offset-md-2 card mb-5 px-3 py-2" style="box-shadow: rgba(0, 0, 0, 0.24) 0px 3px 8px;">
                    ...
                    <div class="float-right w-100 text-right">
                        <a href="admin_announcement.php" class="btn btn-secondary btn-sm text-white mt-3 mb-3 pl-4 pr-4 pb-1 pt-1">Back to Announcements</a>
                        <button type="submit" class="btn btn-success btn-sm mt-3 mb-3 pl-4 pr-4 pb-1 pt-1" name="a_announcement_submit" id="imp">Publish</button>
                    </div>
                </div>
            </form>
        </div>
