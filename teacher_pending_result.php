...
<body>
    <div class="container mt-4 col-lg-12 col-md-12 col-sm-12">
        <div class="row mt-4">
            <div class="col-md-12">
                <h4 class="text-primary ml-3 pt-2">Student Result - Repeat Submission</h4>
            </div>
        </div>
    </div>
            ...
            <div class="align-center text-center">
                <table class="table data-table" id="resultSubmissionTable">
                    ...
                    <tbody>
                        ...
                        $sql_user_stdBatch = date('y') - 1;
                        // $select = "SELECT * FROM `userdata` WHERE semester='{$c_row['semesters']}' AND section = '{$_GET['sec']}' AND student_batch='{$user_stdBatch}'";
                        // $select = "SELECT * FROM `userdata` WHERE section = '{$_GET['sec']}' AND student_batch='{$user_stdBatch}'";



                        // $select = "SELECT * FROM `userdata` WHERE section = '{$_GET['sec']}'";



                        $select = "SELECT * FROM `userdata` INNER JOIN `course_total_marks` ON userdata.user_id=course_total_marks.user_id 
                        AND course_total_marks.student_batch= '{$sql_user_stdBatch}'  
                        AND ((course_total_marks.obtain_marks / course_total_marks.total_marks * 100) < 50) AND userdata.section = '{$_GET['sec']}'
                         AND course_id='{$_GET['id']}' GROUP BY userdata.user_id";
                        // AND course_total_marks.teacher_id= '{$_SESSION['id']}' AND course_id='{$_GET['id']}' GROUP BY userdata.user_id";
                        ...
                        ?>
                            <tr>
                                ...
                                <td>
                                    ...
                                    // $t_assign_query = "SELECT SUM(total_marks) AS value_sum FROM `total_quiz_marks` WHERE course_id= '{$_GET['id']}' AND section='{$_GET['sec']}' AND student_batch='{$user_stdBatch}'";
                                    // $t_assign_sql = mysqli_query($con, $t_assign_query);
                                    // $t_assign_row = mysqli_fetch_assoc($t_assign_sql);
                                ...
                                <td>
                                    ...
                                    $assign_query = "SELECT SUM(marks) AS value_sum FROM `marks` WHERE user_id= '{$result['user_id']}' AND Course= '{$_GET['id']}' AND student_batch='{$user_stdBatch}'";
                                    ...
                                    $assign_query = "SELECT SUM(obtain_marks) AS value_sum FROM `total_quiz_marks` WHERE student_id= '{$result['user_id']}' AND course_id= '{$_GET['id']}' AND student_batch='{$user_stdBatch}'";
                                    $assign_sql = mysqli_query($con, $assign_query);
                                    $assign_row = mysqli_fetch_assoc($assign_sql);
                                    $a_get_quiz = $assign_row['value_sum'];
                                    // Get Obtained GDB Result
                                    $assign_query = "SELECT SUM(obtain_marks) AS value_sum FROM `discussion_answers` WHERE student_id= '{$result['user_id']}' AND course_id= '{$_GET['id']}' AND student_batch='{$user_stdBatch}'";
                                    $assign_sql = mysqli_query($con, $assign_query);
                                    $assign_row = mysqli_fetch_assoc($assign_sql);
                                ...
                                <form action="teacher_pending_result.php?id=<?php echo $_GET['id'] ?>&sec=<?php echo $_GET['sec'] ?>" method="POST">
                                    <td>
                                        ...
                                        <input type="hidden" class="hidden" id="r_std_userBatch" name="r_std_userBatch" value="<?php echo $user_stdBatch ?>">
                                        <?php
